(function ($) {
	...
	$.fn.togglerTabs = function () {
		...
		tabsContent.each(function (index) {
			var self = $(this);
			self.children().first().attr('tabindex', '0');
		...
		// These are the tabs, the first one being selected.
		var tablistItems = tabs.find('.tabs__item a');
		tablistItems.attr('role', 'tab');
		tablistItems.first().attr({
			'aria-selected': 'true',
			'tabindex': 0
		}).parent('.tabs__item').addClass('tabs__item--selected');
