...
namespace Appva.Mcss.Admin.Domain.Repositories
{
    ...
    public interface IPatientRepository : IRepository<Patient>, IUpdateRepository<Patient>
    {
        /// <summary>
        ...
        /// <returns></returns>
        Paged<PatientModel> Search(SearchPatientModel model, IList<Guid> schedulePermissions, int page = 1, int pageSize = 10);
    }
    ...
    public sealed class PatientRepository : Repository<Patient>, IPatientRepository
    {
        ...
        /// <inheritdoc />
        public Paged<PatientModel> Search(SearchPatientModel model, IList<Guid> schedulePermissions, int page = 1, int pageSize = 10)
        {
            var pageQuery   = PageQuery.New(page, pageSize);
            Patient patient = null;
            var query       = this.Context
                .QueryOver<Patient>(() => patient)
            ...
            query
                .OrderByAlias(() => dto.HasUnattendedTasks).Desc
                .ThenByAlias (() => dto.LastName).Asc
                .TransformUsing(Transformers.AliasToBean<PatientModel>());
            var count = query.RowCount();
            var items = query.Skip(pageQuery.Skip).Take(pageQuery.PageSize).List<PatientModel>();
            return Paged<PatientModel>.New(pageQuery, items, count);
        }
