			...
			function showDialogIfDirty(e) {
				if (form.data('dirty') === 'true' && form.is(':visible')) {
					...
					confirmDialog.dialogElement.on('click', '[data-dialog-action="close"]', function () {
						...
						// In the case of form-closing this is to close the dialog that the form is in.
						// The reason why we have a special event for links (A-elements), is that
						// .trigger() doesn't seem to initiate navigation on a link for some reason.
						if (triggeringEvent.type === 'formclosing') {
							$(triggeringEvent.target).trigger('close.netrdialog');
						}
						else if (triggeringEvent.target.nodeName === 'A') {
							triggeringEvent.target.click();
						}
						else {
