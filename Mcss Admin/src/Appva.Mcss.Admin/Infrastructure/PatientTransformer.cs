        ...
        public IList<PatientViewModel> ToPatientList(IList<Patient> patients)
        {
            ...
            foreach (var patient in patients)
            {
                ...
                retval.Add(new PatientViewModel()
                {
                    ...
                    SeniorAlerts = tenantHasSeniorAlert ? patient.SeniorAlerts.ToList().Select(x => new TaxonItem(x.Id, x.Name, x.Description, x.Path, x.Type, x.Weight, null, x.IsActive)).ToList<ITaxon>() : null
                });
            }
            return retval;
        }
