...
namespace Appva.Mcss.Admin.Infrastructure.Attributes
{
    ...
    public class DispatchAsyncAttribute : ActionFilterAttribute
    {
        #region Variables.
        ...
        public DispatchAsyncAttribute(Type requestType)
        ...
        public DispatchAsyncAttribute(string action, string controller)
        ...
        private DispatchAsyncAttribute(Type requestType, string action, string controller, AttributeAction attributeAction)
        {
            this.Order = 1000;
        ...
        private void CreateRequestByParameter(ActionExecutingContext filterContext)
        {
            ...
            var model = Handle(handler, request);
        ...
        private void CreateRequestByParameterAndRedirect(ActionExecutingContext filterContext)
        {
            //Requires.ValidState(filterContext.ActionParameters.ContainsKey("request"), "Parameter 'request' does not exist");
            var request = filterContext.ActionParameters["request"];
            var routeValues = this.Handler(request).Handle(request).Result;
        ...
        private void CreateRequestAutomatically(ActionExecutingContext filterContext)
        {
            ...
            var handler = this.Handler(request);
            filterContext.Controller.ViewData.Model =  handler.Handle(request).Result;
        }
        ...
        private IAsyncRequestHandler Handler(object request)
        {
            var modelType = request.GetType().GetInterfaces()[0].GetGenericArguments()[0];
            var handlerType = typeof(IAsyncRequestHandler<,>).MakeGenericType(request.GetType(), modelType);
            ...
            return handler as IAsyncRequestHandler;
        }

        private Task<object> Handle(IAsyncRequestHandler handler, object request)
        {
