    ...
    Dialog.prototype = {
        ...
        open: function () {
            ...
            $(document).bind('click.netrdialog touchend.netrdialog', function (e) {
                ...
                // If it _isn't_ in the dialog element _or_ the dialog element itself
                if (!jQuery.contains(self.dialogElement.get(0), e.target) && (self.dialogElement.get(0) !== e.target)) {

                    // If a datepicker is present
                    if ($('.ui-datepicker').length) {

                        // Then check if the event is is  within that datepicker
                        if (jQuery.contains($('#ui-datepicker-div').get(0), e.target) || ($('#ui-datepicker-div').get(0) === e.target) || $(e.target).data('event') === 'click') {
                            return;
                        }
                    }

                    // Allow clicks on exit confirmation
                    if (!jQuery.contains($('#exit-confirmation').get(0), e.target) && ($('#exit-confirmation').get(0) !== e.target)) {
                        return;
                    }
                    // close dialog
                    e.preventDefault();
                            ...
                            if (form) {
                                ...
                                dialog.contentElement.on('click', '[data-dialog-close]', function () {
                                    if (firmDirty === false) {
                                        ...
                                        $('body').find($('#form-leave')).click(function (e) {
                                            e.preventDefault();
                                            dialog.close();
                                        });
                                    }
