(function ($) {
	...
	$.fn.toggleByInput = function (settings) {
		...
		this.each(function () {
			...
			var targets = $('#' + formControl.data(config.toggleTargetsDataAttribute).join(', #'));

			// add Aria attributes
			formControl.attr('aria-controls', formControl.data(config.toggleTargetsDataAttribute).join(' '));
