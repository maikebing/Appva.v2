(function ($) {
	...
	$.fn.numberSpinner = function (settings) {
		...
		this.each(function () {
            ...
            var decrease = $('<button class="button button--icon decrease" tabindex="-1"><span class="visually-hidden">' + netr.string.translate('spinner.decrease') + ' ' + label + '</span><svg class="icon icon--small" aria-hidden="true"><use xlink:href="/Assets/images/icons/icon.sprite.svg#arrow--fat-icon" /></svg></button>');
            var increase = $('<button class="button button--icon increase" tabindex="-1"><span class="visually-hidden">' + netr.string.translate('spinner.increase') + ' ' + label + '</span><svg class="icon icon--small" aria-hidden="true"><use xlink:href="/Assets/images/icons/icon.sprite.svg#arrow--fat-icon" /></svg></button>');
			...
			if (element.data('direction') === 'vertical') {
				...
				decrease.find('svg').addClass('icon--s');
			}
			else {
				decrease.find('svg').addClass('icon--w');
				increase.find('svg').addClass('icon--e');
			}
